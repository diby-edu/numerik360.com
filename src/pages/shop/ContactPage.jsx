...
async function fetchContactSettings() {
  const keys = ['contact_email', 'contact_phone', 'contact_address', 'whatsapp_number', 'social_facebook', 'social_instagram']
...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-theme-bg">
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-primary to-primary-dark text-white py-16 px-4 text-center">
        <h1 className="text-4xl font-black mb-3">Contactez-nous</h1>
        <p className="text-white/80 max-w-lg mx-auto">
          Une question ? Un souci ? Notre équipe est là pour vous aider.
        </p>
      </section>

      <div className="max-w-5xl mx-auto px-4 py-14 grid md:grid-cols-5 gap-10">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {s.contact_phone && (
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <a href={`tel:${s.contact_phone}`} className="font-semibold text-gray-900 hover:text-primary transition-colors">
                  {s.contact_phone}
                </a>
              </div>
            </div>
          ...
          {s.contact_email && (
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <a href={`mailto:${s.contact_email}`} className="font-semibold text-gray-900 hover:text-primary transition-colors">
                  {s.contact_email}
                </a>
              </div>
            </div>
          ...
          {s.contact_address && (
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-0.5">Adresse</p>
                <p className="font-semibold text-gray-900">{s.contact_address}</p>
              </div>
            </div>
          )}
          ...
          {cleanWa && (
            <a
              href={`https://wa.me/${cleanWa}?text=${encodeURIComponent('Bonjour, je souhaite vous contacter.')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 bg-[#25D366] text-white rounded-xl px-4 py-3 font-bold text-sm hover:opacity-90 transition-opacity w-full"
            >
          ...
          {!s.contact_phone && !s.contact_email && !s.contact_address && (
            <div className="bg-gray-50 rounded-xl p-4 text-gray-400 text-sm">
              Coordonnées à configurer dans l'admin → Paramètres.
            </div>
          )}
        </div>
        ...
        <div className="md:col-span-3">
          ...
          {sent ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-green-700 text-center">
              ...
              <p className="text-sm">Nous vous répondrons dans les meilleurs délais.</p>
              <button onClick={() => setSent(false)} className="mt-4 text-sm text-green-600 hover:underline">
